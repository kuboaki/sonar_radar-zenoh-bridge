...
**根本的な課題**: タイムアウトのマージンで調整するのは対症療法で、本質的には実機とシムの旋回位置が互いに独立して自走しており、同期していないことが問題。将来的には、followerがleaderのエンコーダ角度を読んで追従旋回する(自分のペースで回らず、leaderの動きに従属する)設計に変えることで、両者の位置がずれる余地自体を無くせる可能性がある(Zenoh経由の遅延など別の懸念は残る、未着手)。

### WAIT_FOR_DETECTED_GRACE(2026-08-05追加): 位置追従の代替となる二段階タイムアウト
